///_Este bot foi criado pelo duds caso vá usar
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:duds\n' // Seu nome
            + 'ORG:DUDS bot;\n' // Nome do bot
            + 'TEL;type=CELL;type=VOICE;waid=556199316185:+55 61 9931-6185\n' //Seu número Whatsapp
            + 'END:VCARD'
         ...
         //   pushname = client.contacts[sender] != undefined ? client.contacts[sender].vname || client.contacts[sender].notify : undefinedconst ismod = mod.includes(sender)
           ...
           return console.log(color('[BAN] Ignorando comando', 'blue'), color(moment.tz('America/Sao_Paulo').format('HH:mm:ss'), 'yellow'), color(`${command}`),'DE:', color(pushname))}
			...
			function addMetadata(packname, author) {	
				if (!packname) packname = 'PRIVATE'; if (!author) author = 'duds';	
...
client.sendMessage('556199316185@s.whatsapp.net', options, text, {quoted: mek})
...
case 'pack':
                    ...
                    client.sendMessage(from, pack18(prefix) , text, {quoted: mek, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "By duds", 'jpegThumbnail': fs.readFileSync('kk/sticker/botlogo.webp')}}}})					
                break
case 'utils':
...
case 'antifake':
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* duds\n*WPP:* wa.me/+556199316185\n*INSTA:* @edu_matos12\n\n\nEspero que tenham gostado do bot '})
					  client.sendMessage(from, 'Ctt do meu dono ai Wa.me/556199316185 , pfv n flode',MessageType.text, { quoted: mek} )
                    break
...
case 'casal':
      ...
      if (!isGroup) return reply(mess.only.group)
                        ...
                        teks = `
                  Paradas!🤚🤚\n\n1=🤚🤭@${o1.jid.split('@')[0]}🤚🤭\n\n\n2=🤚🤭@${o2.jid.split('@')[0]}🤚🤭\n\n\n3=🤚🤭@${o3.jid.split('@')[0]}🤚🤭\n\n\n4=🤚🤭@${o4.jid.split('@')[0]}🤚🤭\n\n\n5=🤚🤭@${o5.jid.split('@')[0]}🤚🤭\n\n\nMultas por serem gostosas dms😳😊 by Bot`
                        member.push(o1.jid)
                        member.push(o2.jid)
                        member.push(o3.jid)
                        member.push(o4.jid)
			     ...
			     case 'fig':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('duds', authorname)} ${ran} -o ${ran}`, async (error) => {
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
								...
								exec(`webpmux -set exif ${addMetadata('duds', authorname)} ${ranw} -o ${ranw}`, async (error) => {
									if (error) return reply(mess.error.stick)
